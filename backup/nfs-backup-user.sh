...
NFS_PATH=$(oc get pv "$PV_NAME" -o jsonpath='{.spec.nfs.path}' 2>/dev/null)

if [ -z "$NFS_PATH" ]; then
  echo "ERROR: PVC '$PVC_NAME' is not backed by NFS"
  exit 1
fi
...
echo "COS dest:  s3://pytorch-nfs-backup/$NFS_SUBDIR/"
echo ""
...
oc delete job "nfs-backup-$USERNAME" -n "$USERNAME" --ignore-not-found

cat <<EOF | oc apply -f -
apiVersion: batch/v1
kind: Job
metadata:
  name: nfs-backup-$USERNAME
  namespace: $USERNAME
spec:
  backoffLimit: 2
      ...
      - name: backup
        ...
        - |
          ...
          aws --endpoint-url https://s3.ca-tor.cloud-object-storage.appdomain.cloud \
            s3 sync /source/ s3://pytorch-nfs-backup/$NFS_SUBDIR/ \
            --no-follow-symlinks \
            --only-show-errors
...
echo "  oc logs -f job/nfs-backup-$USERNAME -n $USERNAME"
echo ""
echo "When complete, clean up with:"
echo "  oc delete job nfs-backup-$USERNAME -n $USERNAME"
