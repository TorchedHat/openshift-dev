#!/bin/bash
# Migrate a single user's data from the old CephFS RWX PVC
...
#
# CephFS is RWX network storage, so no node pinning is needed. Still, scale the
# user's workload down first so the source doesn't change mid-copy:
...
echo "Dest:    $NEW_PVC (ocs-storagecluster-cephfs, $SIZE)"
echo ""
...
oc delete job "cephfs-migrate-$USERNAME" -n "$USERNAME" --ignore-not-found

cat <<EOF | oc apply -f -
apiVersion: batch/v1
kind: Job
metadata:
  name: cephfs-migrate-$USERNAME
  namespace: $USERNAME
spec:
  backoffLimit: 2
...
echo "  oc logs -f job/cephfs-migrate-$USERNAME -n $USERNAME"
echo ""
...
echo "  oc delete job cephfs-migrate-$USERNAME -n $USERNAME"
